...
try:
    ...
    # When running with exec() or in interactive mode
    script_path = "src/onnx_export.py"
    current_dir = os.getcwd()
    if os.path.exists(os.path.join(current_dir, script_path)):
        BASE_DIR = Path(current_dir)
    else:
        # Fallback to absolute path
        BASE_DIR = Path("D:/Personal/IITP/Projects/SLM/solution")
        if not BASE_DIR.exists():
            BASE_DIR = Path(os.path.dirname(os.path.dirname(os.path.abspath(script_path))))
...
class ONNXExporter:
    ...
    def _load_config(self) -> Dict:
        """Load model configuration from JSON file and initialize IndicSLMConfig"""
        from src.model import IndicSLMConfig
        # Load config using HuggingFace PretrainedConfig
        config_obj = IndicSLMConfig.from_json_file(str(self.config_path))
        logger.info(f"Loaded model configuration from {self.config_path}")
        return config_obj
    
    def _load_tokenizer(self) -> SLMTokenizer:
        """Load the tokenizer"""
        # Load tokenizer using the provided config_path
        config_file = Path(self.config_path)
        if not config_file.exists():
            raise FileNotFoundError(f"Tokenizer config file not found: {config_file}")
        tokenizer = SLMTokenizer(config_path=str(config_file))
        logger.info(f"Loaded tokenizer using config: {config_file}")
        return tokenizer
    
    def _load_model(self) -> nn.Module:
        """Load the PyTorch model"""
        from src.model import IndicSLM
        try:
            # Load model and weights via HuggingFace API
            model = IndicSLM.from_pretrained(
            ...
            model.eval()
            logger.info(f"Loaded model from {self.model_path} via from_pretrained")
            return model
        except Exception as e:
            logger.error(f"Failed to load model via from_pretrained: {e}")
            raise
